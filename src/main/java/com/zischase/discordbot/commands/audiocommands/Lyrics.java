...
public class Lyrics extends Command {
    ...
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            Element result = doc.select("#rso > div:nth-child(1) > div > div.r > a").first();
            String lyricsURL;
            ...
            for (int i = 0; i < 20; i++) {
                lyricsElement = doc.select("body > div.container.main-page > div > div.col-xs-12.col-lg-8.text-center > div:nth-child(" + i + ")").first();

                if (lyricsElement == null)
                    continue;

                if (lyricsElement.html().contains("<br>"))
                    break;
            }
        } catch (IOException ignored){}

        if (lyricsElement == null) {
            ctx.getChannel().sendMessage("Sorry, i couldn't find anything :c").queue();
            return;
        }
        ...
        for (String str : lyrics) {
            ...
            if (str.length() > 2000) {
                ...
                while (m.find())
                    ctx.getChannel().sendMessage(embed.setDescription(m.group()).build()).queue();
            }
            else
