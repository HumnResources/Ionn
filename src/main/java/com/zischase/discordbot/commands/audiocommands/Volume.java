...
public class Volume extends Command
{
	...
	private final int DEFAULT_VOL = Integer.parseInt(Config.get("VOLUME"));
	
	private int setVolume = DEFAULT_VOL;
	...
	public void setVolume(Guild guild)
	{
		...
		setVolume = Jdbi.create(PostgreSQL::getConnection).withHandle(handle ->
		{
		...
		if (args.get(0).matches("\\d+"))
		{
			...
			if (validNum)
			{
				Jdbi.create(PostgreSQL::getConnection)
						.useHandle(handle ->
								handle.execute("UPDATE guild_settings SET volume = ? WHERE guild_id = ?", args.get(0), guild
										.getId())
						);
