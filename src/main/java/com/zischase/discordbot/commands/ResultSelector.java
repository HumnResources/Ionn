...
public class ResultSelector
{
	...
	private ListenerAdapter addEventListeners()
	{
		return new ListenerAdapter()
		{
			...
			{
				if (event.getMember() != initiator && Objects.requireNonNull(event.getGuild()).getId().equals(guildID))
					return;
			...
			public void onStringSelectInteraction(@org.jetbrains.annotations.NotNull StringSelectInteractionEvent event)
			{
				...
				if (!event.getMember().getId().equals(initiator.getId()) || event.getHook().isExpired() || !event.getMessage().getId().equals(messageID))
					return;
